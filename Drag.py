...
class Balloon():
    ...
    def update_pos(self,index_x,index_y,middle_x, middle_y, fingers, l, rectTrashCan):
        if not Balloon.grabbed or self.picked:
            ...
            if self.rectBalloon.collidepoint(index_x, index_y) or self.rectBalloon.collidepoint(middle_x, middle_y):
                if fingers[1] == 1 and fingers[2]==1 and l<65:
                    self.picked = True
            if self.picked: 
                self.rectBalloon.x = (index_x+middle_x)/2 - self.w_balloon/2
                self.rectBalloon.y = (index_y+middle_y)/2 - self.ht_balloon/2
            if self.picked and (rectTrashCan.collidepoint(index_x, index_y) or rectTrashCan.collidepoint(middle_x, middle_y)) and l>40:#improve this by checking if balloon is here
                self.goal = True
                self.end=True
    ...
    def update_pos_new(self,index_x,index_y,middle_x, middle_y, fingers, l, rectTrashCan, rectRecycling):
        if not Balloon.grabbed or self.picked:
            ...
            elif self.picked and (rectRecycling.collidepoint(index_x, index_y) or rectRecycling.collidepoint(middle_x, middle_y)) and l>40:#improve this by checking if balloon is here
                self.goal = True
                self.end=True
                self.speed += 1 #this is no longer helpful if not respawning
                self.picked=False
    ...
    while running:
        ...
        if time_left <= 0:
            ...
            # lmList, _ = detector.findPosition(img)
                ...
                deleted =[]
                for i in range(len(Balloons)):
                    Balloons[i].update_pos(index_x,index_y,middle_x, middle_y, fingers, l, rectTrashCan)
    ...
    while running:
        ...
        if time_left <= 0:
            ...
            # lmList, _ = detector.findPosition(img)
                ...
                deleted =[]
                for i in range(len(Balloons)):
                    Balloons[i].update_pos_new(index_x,index_y,middle_x, middle_y, fingers, l, rectTrashCan, rectRecycling)
                    if Balloons[i].goal==True:
